...
describe('Get all customers by organization (E2E)', () => {
  ...
  test('[GET] /organizations/:organizationId/customers/id/:id - should be able to get an customer that exists', async () => {
  ...
  test('[GET] /organizations/:organizationId/customers/id/:id - should return 404 for non existing customer', async () => {
    const user = await userFactory.makePrismaUser();
    const accessToken = await userFactory.makeToken(user.id.toString());
    const organization = await organizationFactory.makePrismaOrganization({
      ownerId: user.id,
    ...
    const response = await request(app.getHttpServer())
      .get(`/organizations/${organization.id}/customers/id/123456`)
      .set('Authorization', `Bearer ${accessToken}`);
  ...
  test('[GET] /organizations/:organizationId/customers - should return 404 in non existing organization', async () => {
    ...
    const response = await request(app.getHttpServer())
      .get(`/organizations/999999/customers/id/123456`)
      .set('Authorization', `Bearer ${accessToken}`);
